...
def inflate_slide_inference(model, im, crop_size, stride, batch_size=1):
    """
    膨胀滑窗预测
    ...
    # (计算增加右下边padding和四周padding之后的大图的移动步数)
    rows = int(np.ceil(h_im / h_stride) + 2)
    cols = int(np.ceil(w_im / w_stride) + 2)

    # 构造新的大图
    new_img_shape = [im.shape[0], im.shape[1], rows * h_stride, cols * w_stride]
    new_img = np.full(new_img_shape, 0, dtype="float32")
    new_img[:, :, h_stride // 2:h_stride // 2 + h_im, w_stride // 2:w_stride // 2 + w_im] = im

    bs_patch = list()
    cnt = 0
    index = list()
    final_logit = None
    # final_logit = np.full(new_img_shape, 0, dtype="float32")
    count = np.zeros([1, 1, new_img_shape[-2], new_img_shape[-1]])  # 用来记录像素点是否已推理
    for r in tqdm(range(rows)):  # 逐行
        for c in range(cols):  # 逐列
            ...
            im_crop = new_img[:, :, h1:h2, w1:w2]  # 切片

            bs_patch.append(im_crop)  # bs_patch：一个列表，储存一个 batch 的小图
            cnt += 1  # 计数
            index.append((r, c))  # index：一个列表，储存一个 batch 的小图的二元索引
            if cnt == batch_size:  # 达到 batch size 时，开始一并推理
                try:
                    bs_patch_array = np.concatenate(bs_patch, axis=0)  # 将一个 batch 中的多个小图在 0 维上拼接到一起
                except:
                    print(rows, cols)
                bs_patch_tensor = paddle.to_tensor(bs_patch_array, dtype='float32')
                patch_pre = model(bs_patch_tensor)  # 核心：使用 self.testBatch 方法进行多图推理
                patch_pre = patch_pre[0].numpy()
                pad = h_stride // 2
                patch_pre = patch_pre[:, :, pad: pad + h_stride, pad: pad + w_stride]  # 保留中间部分，去除边缘
                if final_logit is None:
                    final_logit = np.zeros([1, patch_pre.shape[1], rows * h_stride, cols * w_stride])
                # 将测试结果拼接回去
                for k, (i, j) in enumerate(index):
                    h1 = i * h_stride + h_stride // 2
                    w1 = j * w_stride + w_stride // 2
                    h2 = h1 + h_stride
                    w2 = w1 + w_stride
                    final_logit[:, :, h1:h2, w1:w2] += patch_pre[k, :, :, :]
                    count[:, :, h1:h2, w1:w2] += 1

                index.clear()  # 将 index 列表清空以便下一 batch 的推理
                bs_patch.clear()  # 将 bs_patch 列表清空以便下一 batch 的推理
                cnt = 0  # 将 cnt 清零以便下一 batch 的推理

    if np.sum(count == 0) != 0:
        raise RuntimeError(
            'There are pixel not predicted. It is possible that stride is greater than crop_size'
        )
    final_logit = final_logit[:, :, h_stride // 2:h_stride // 2 + h_im,
                  w_stride // 2:w_stride // 2 + w_im]  # 去除边缘padding
    final_logit = paddle.to_tensor(final_logit)
    return final_logit


def ma_slide_inference(model, im, crop_size, stride, batch_size):
    # print('slide inference batch size is {}'.format(batch_size))
    h_im, w_im = im.shape[-2:]
    w_crop, h_crop = crop_size
    w_stride, h_stride = stride
    # calculate the crop nums
    ...
    for r in tqdm(range(rows)):
        for c in range(cols):
            ...
            bs_patch.append(im_crop)
            index.append((h1, h2, w1, w2))
            if r==1 and c == 1:
                print('Flops is:')
                flops_tensor = paddle.to_tensor(bs_patch, dtype='float32')
                print(slim.analysis.flops(model, [im_crop]))
            if len(bs_patch) == batch_size:
                bs_patch_tensor = paddle.to_tensor(np.concatenate(bs_patch, axis=0), dtype='float32')
                patch_pre = model(bs_patch_tensor)  # 核心：使用 self.testBatch 方法进行多图推理
                patch_pre = patch_pre[0].numpy()
                if final_logit is None:
                    final_logit = np.zeros([1, patch_pre.shape[1], h_im, w_im])
                for k, (h1, h2, w1, w2) in enumerate(index):
                    final_logit[:, :, h1:h2, w1:w2] += patch_pre[k, :, :, :]
                    count[:, :, h1:h2, w1:w2] += 1
                index.clear()
                bs_patch.clear()
    ...
    final_logit = final_logit / count   # 相当于加起来除以被推理的次数，重叠部分会被多次推理
    final_logit = paddle.to_tensor(final_logit)
    return final_logit
...
def slide_inference(model, im, crop_size, stride):
    ...
    try:
        ...
        print('cuda memory is limited, use cpu instead')
    return final_logit
...
def inference(model,
    ...
    if not is_slide:
        ...
        logit = ma_slide_inference(model, im, crop_size=crop_size, stride=stride, batch_size=batch_size)
    if hasattr(model, 'data_format') and model.data_format == 'NHWC':
